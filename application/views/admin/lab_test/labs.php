            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title titlefix"> Lab Bill</h3>
                        <div class="box-tools pull-right">
                            <?php if ($this->rbac->hasPrivilege('lab_bill', 'can_add')) {?>
                            ...
                            <?php }if ($this->rbac->hasPrivilege('lab_test', 'can_view')) {?>
                            <a data-toggle="" href="<?php echo base_url(); ?>admin/labtest/search"
                    ...
                    <div class="box-body">
                        ...
                        <div class="">
                            <table class="table table-striped table-bordered table-hover ajaxlist" id="testreport"
                                ...
                                data-export-title="<?php echo $this->lang->line('radiology_test_reports'); ?>">
            ...
            <form id="bill" accept-charset="utf-8" method="post">
                                ...
                                <div class="input-group">
                                    ...
                                    <div class="input-group-btn">
                                        <button class="btn btn-default btn-group-custom" type="button"
                                            id="search_prescription"><i class="fa fa-search"></i>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-lg-2 col-md-3 col-sm-4">
                                    <label><?php echo $this->lang->line('bill_no'); ?><input readonly name="bill_no"
                                            id="billno" type="text" class="transparentbg-border" />
                                        <span class="text-danger"><?php echo form_error('bill_no'); ?></span></label>
                                </div>
                                <div class="col-lg-3 col-md-4 col-sm-4">
                                    <label><?php echo $this->lang->line('case_id'); ?>
                                        <input readonly name="case_reference_id" id="case_reference_id" type="text"
                                            class="transparentbg-border" />
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="gender" id="addformgender">
                                    ...
                                    <?php foreach ($genderList as $key => $value) {?><option value="<?php echo $key; ?>"
                            ...
                            <div class="form-group">
                                ...
                                <div style="clear: both;overflow: hidden;">
                                    ...
                                    <input type="number" id="age_month"
                                        placeholder="<?php echo $this->lang->line('month'); ?>" name="age[month]"
                                        value="0" class="form-control patient_age_month"
                                        style="width: 36%;float: left; margin-left: 4px;">
                                    <input type="number" id="age_day"
                                        placeholder="<?php echo $this->lang->line('day'); ?>" name="age[day]" value="0"
                ...
                <div class="modal-footer sticky-footer">
                    <div class="pull-right">
                        ...
                        <button type="button" onclick="addTotal()" class="btn btn-info" autocomplete="off"><i
                        ...
                        <button type="submit" name="save"
                            ...
                            id="billsave" class="btn btn-info pull-right"><i class="fa fa-check-circle"></i>
                            <?php echo $this->lang->line('save'); ?></button>
                        <button type="submit" name="save_print" style="display: none; margin-right:2px;"
...
$(document).on('click', '.assigntest', function() {
    ...
    $this.button('loading');
    $.ajax({
        url: '<?php echo base_url(); ?>admin/labtest/assigntestradio',
        type: "POST",
        dataType: 'json',
        beforeSend: function() {
            $this.button('loading');
...
$(document).on('click', '.edit_radiology', function() {
    ...
    $this.button('loading');
    $.ajax({
        url: base_url + 'admin/labtest/editradiology',
        type: "POST",
        data: {
            'id': recordId
        },
...
$(document).on('click', '.delete_radiology', function() {
    if (confirm('<?php echo $this->lang->line("delete_confirm") ?>')) {
        ...
        $this.button('loading');
        $.ajax({
            url: base_url + 'admin/labtest/deleteRadiologyBill',
            type: "POST",
            data: {
                'id': recordId
            },
...
function gettestradiodetails(testid, id) {
    $.ajax({
        type: "POST",
        url: base_url + "admin/labtest/gettestradiodetails",
        data: {
            'id': testid
        },
        dataType: 'json',
...
$(document).on('click', '.add-record', function() {
    ...
    var div = "<td><input type='hidden' id='total_rows' name='total_rows[]' value='" + total_rows +
        "'><input type='hidden' name='inserted_id_" + total_rows +
        "' value='0'><select class='form-control test_name select2' style='width:100%'  onchange='gettestradiodetails(this.value," +
        total_rows + ")' name='test_name_" + total_rows +
        "' ><option value='<?php echo set_value('test_name_id'); ?>'><?php echo $this->lang->line('select') ?></option>" +
        template + "</select></td><td><input type='text' name='reportday_" + total_rows + "' id='reportday_" +
        total_rows +
        "'  class='form-control text-right days' readonly></td><td><input type='text' name='reportdate_" +
        total_rows + "' id='reportdate_" + total_rows +
        "'  class='form-control text-right report_date'></td><td><div class='input-group'><input type='text' name='taxpercent_" +
        total_rows + "' id='taxpercent_" + total_rows +
        "'  class='form-control text-right right-border-none taxpercent' autocomplete='off' readonly><span class='input-group-addon'> %</span></div></td><td><input type='text' name='amount_" +
        total_rows + "' id='amount_" + total_rows +
        "'  class='form-control text-right amount' readonly><input type='hidden' name='taxamount_" +
        total_rows + "' id='taxamount_" + total_rows +
        "'  class='form-control text-right taxamount' readonly></td>";

    var row = "<tr id='row" + total_rows + "'>" + div + "<td><button type='button' data-row-id='" + total_rows +
        "' class='closebtn delete_row'><i class='fa fa-remove'></i></button></td></tr>";
    $('#tableID').append(row);
    updateDate();
    $('.test_name').select2();
    total_rows++;
...
$(document).ready(function(e) {
    ...
    $(document).on('submit', '#bill', function(e) {
        ...
        $.ajax({
            url: base_url + 'admin/labtest/addBill',
            type: "POST",
...
function viewDetailReport(id, radiology_id) {
    $.ajax({
        url: '<?php echo base_url() ?>admin/labtest/getReportDetails/' + id + '/' + radiology_id,
        type: "GET",
        data: {
            id: id
        },
        success: function(data) {
            ...
            $('#edit_deletereport').html(
                "<?php if ($this->rbac->hasPrivilege('add_radio_patient_test_reprt', 'can_view')) {?><a href='#' data-toggle='tooltip' onclick='printData(" +
                id +
...
function printData(id) {
    $.ajax({
        url: base_url + 'admin/labtest/getBillDetails',
        type: "POST",
        data: {
            'id': id
        },
...
function viewDetailbill(id) {
    ...
    $.ajax({
        url: '<?php echo base_url() ?>admin/labtest/getradiobilldetails',
        type: "POST",
        data: {
            'id': id
        },
...
function deleterecord(id) {
    var url = '<?php echo base_url() ?>admin/labtest/deletetestbill/' + id;
...
function editTestReport(id) {
    $.ajax({
        url: '<?php echo base_url(); ?>admin/labtest/getRadiologyReport',
        type: "POST",
        data: {
            id: id
        },
...
$(document).ready(function(e) {
    $("#updatetest").on('submit', (function(e) {
        ...
        e.preventDefault();
        $.ajax({
            url: '<?php echo base_url(); ?>admin/labtest/updateTestReport',
            type: "POST",
            data: new FormData(this),
            dataType: 'json',
            contentType: false,
...
$(document).on('submit', '#form-sample-collected', function(e) {
    ...
    $.ajax({
        url: base_url + 'admin/labtest/updatecollection',
        type: "POST",
        data: form.serialize(),
        dataType: 'json',
        beforeSend: function() {
...
$(document).on('submit', '#form-report_param', function(e) {
    ...
    var url = form.attr('action');
    $.ajax({
        url: base_url + 'admin/labtest/updatereportparam',
        type: "POST",
        data: new FormData(this),
        dataType: 'json',
        processData: false,
...
function getPrescriptionData() {
    ...
    $.ajax({
        url: baseurl + 'admin/labtest/prescriptionBill',
        type: "POST",
        data: {
        ...
        success: function(res) {
            ...
            if (res.record_available == 1) {
                ...
                var option = new Option(res.patient_name + " (" + res.patient_id + ")", res.patient_id,
                    true, true);
                $("#bill .patient_list_ajax").append(option).trigger('change');
            }
            updateDate();
...
function getPayments(record_id) {
    var payment_modal = $('#addPaymentModal');
    $.ajax({
        url: '<?php echo base_url() ?>admin/labtest/getRadiologyTransaction',
        type: "POST",
        data: {
            'id': record_id
        },
...
$(document).on('click', '.print_receipt', function() {
    ...
    $this.button('loading');
    $.ajax({
        url: '<?php echo base_url(); ?>admin/labtest/printTransaction',
        type: "POST",
        data: {
            'id': record_id
        },
...
function PatientRadiologyDetails(id, btn_obj) {
    ...
    var $this = btn_obj;
    $.ajax({
        url: base_url + 'admin/labtest/getPatientRadiologyDetails',
        type: "POST",
        data: {
            'id': id
        },
...
$(document).on('click', '.add_report', function() {
    ...
    var $this = $(this);
    $.ajax({
        url: base_url + 'admin/labtest/getRadiologyReportDetail',
        type: "POST",
        data: {
            'id': id
        },
...
$(document).on('click', '.print_report', function() {
    ...
    var $this = $(this);
    $.ajax({
        url: base_url + 'admin/labtest/printPatientReportDetail',
        type: "POST",
        data: {
            'id': id
        },
...
$(document).on('click', '.add_collection', function() {
    ...
    var $this = $(this);
    $.ajax({
        url: base_url + 'admin/labtest/getReportCollectionDetail',
        type: "POST",
        data: {
            'id': id
        },
        ...
        success: function(data) {
            ...
            $("#collectionModal .modal-body").find('input[name="radiology_report_id"]').val(data
                .report.id);
            $("#collectionModal .modal-body").find('input[name="radiology_bill_id"]').val(data
                .report.radiology_bill_id);
            $("#collectionModal .modal-body").find('input[name="radiology_center"]').val(data.report
                .radiology_center);
...
(function($) {
    'use strict';
    $(document).ready(function() {
        initDatatable('ajaxlist', 'admin/labtest/getradiologybillDatatable', [], [], 100,
            [{
                    "sWidth": "105px",
                    "aTargets": [-2, -3],
...
$(document).on('click', '.print_parameter', function() {
    ...
    var $this = $(this);
    $.ajax({
        url: base_url + 'admin/labtest/printtestparameterdetail',
        type: "POST",
        data: {
            'id': id
        },
