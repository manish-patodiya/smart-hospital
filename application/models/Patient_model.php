...
class Patient_model extends MY_Model
{
    ...
    public function getAllopdRecord()
    {
        ...
        $from_date = $this->input->post('from_date') ? date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('from_date'))) : date('Y-m-d');
        $to_date = $this->input->post('to_date') ? date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('to_date'))) : date('Y-m-d');
        ...
        $this->datatables
            ...
            ->from('opd_details');
        $this->datatables->where("date(visit_details.appointment_date) between '$from_date' and '$to_date'");
        if ($doc_id) {
            $this->datatables->where('staff.id', $doc_id);
        }
        return $this->datatables->generate('json');
    }
    ...
    public function deleteIpdPatient($ipdid)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $bed = $this->db->select('bed')->where('id', $ipdid)->get('ipd_details')->row()->bed;
        $this->db->where('id', $bed)->update('bed', ['is_active' => 'yes']);
