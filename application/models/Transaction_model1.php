...
class Transaction_model1 extends MY_Model
{
    ...
    {
        $query = $this->db->select('transactions.*,lab_billing.id as lab_billing_id,patients.note as pnote,patients.id as patient_id,patients.patient_name,patients.age,patients.month,patients.gender,patients.id as patient_unique_id,patients.mobileno,patients.email,patients.dob,patients.image,patients.address,staff.name,staff.surname,staff.employee_id')
            ->join("lab_billing", "lab_billing.id = transactions.lab_billing_id")
            ->join("patients", "patients.id = lab_billing.patient_id")
    ...
    {
        $query = $this->db->select('transactions.*,lab_billing.id as lab_billing_id,patients.note as pnote')
            ->join("lab_billing", "lab_billing.id = transactions.lab_billing_id")
            ->join("patients", "patients.id = lab_billing.patient_id")
            ->where("transactions.lab_billing_id", $radiology_billing_id)
            ->order_by("transactions.payment_date", "desc")
            ->get("transactions");
        return $query->result();
    }
    ...
    {
        $query = $this->db->select('IFNULL(sum(amount),0) as total_paid, lab_billing.*')
            ->join("transactions", "lab_billing.id = transactions.lab_billing_id")
            ->join("patients", "patients.id = lab_billing.patient_id")
            ->where("lab_billing.id", $radiology_billing_id)
            ->order_by("transactions.payment_date", "desc")
            ->get("lab_billing ");
        return $query->row();
    }
    ...
    public function getPaidAmountRadiology()
    {
        $query = $this->db->select('transactions.*,lab_billing.id as lab_billing_id,patients.note as pnote')
            ->join("lab_billing", "lab_billing.id = transactions.lab_billing_id")
            ->join("patients", "patients.id = lab_billing.patient_id")
            ->where("transactions.lab_billing_id > 0")
            ->order_by("transactions.payment_date", "desc")
            ->get("transactions");
    ...
    public function transactionRecord($start_date, $end_date, $collect_staff = null)
    {
        ...
        $sql = "select transactions.*, CASE
                   ...
                   WHEN (lab_billing_id IS NOT NULL) THEN 'lab_billing'
    ...
    public function radiologybillRecord($start_date, $end_date, $collect_staff)
    {
        ...
        $sql = "select transactions.id,transactions.type,transactions.payment_mode,transactions.section,'radiology' head,'lab_billing' module_no,transactions.lab_billing_id as module_id,transactions.payment_date,transactions.amount, patients.patient_name,patients.id as patient_id,staff.name,staff.surname,staff.employee_id from transactions LEFT JOIN lab_billing on lab_billing.id = transactions.pathology_billing_id LEFT JOIN patients on patients.id = lab_billing.patient_id LEFT JOIN staff on staff.id = transactions.received_by where date_format(transactions.payment_date,'%Y-%m-%d') >='" . $start_date . "'and date_format(transactions.payment_date,'%Y-%m-%d') <= '" . $end_date . "' and transactions.lab_billing_id is not null " . $condition;
        $this->datatables->query($sql)
            ->searchable('transactions.id,patients.patient_name')
            ->orderable('transactions.id,patients.patient_name,transactions.lab_billing_id,head,transactions.payment_date,transactions.received_by,type,payment_mode,transactions.amount')
            ->sort('transactions.payment_date', 'desc')
            ->query_where_enable(true);
        return $this->datatables->generate('json');
    }
    ...
    public function radiologybillreportsRecord($data)
    {
        ...
        if ($data['test_name'] && $data['test_name'] != "") {
            $condition .= " and lab_report.radiology_id = '" . $data['test_name'] . "' ";
        }

        if ($data['collect_staff'] && $data['collect_staff'] != "") {
            $condition .= " and lab_report.collection_specialist = '" . $data['collect_staff'] . "' ";
        }

        if ($data['radiology_category_id'] && $data['radiology_category_id'] != "") {
            $condition .= " and lab_test.radiology_category_id = '" . $data['radiology_category_id'] . "' ";
        }
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
                ...
                array_push($custom_field_column_array, 'table_custom_' . $i . '.field_value');
                array_push($field_var_array, '`table_custom_' . $i . '`.`field_value` as `' . $custom_fields_value->name . '`');
                $custom_join = ('LEFT JOIN custom_field_values as ' . $tb_counter . ' ON transactions.lab_billing_id = ' . $tb_counter . '.belong_table_id AND ' . $tb_counter . '.custom_field_id = ' . $custom_fields_value->id);
                $i++;
            }
        }
        ...
        $sql = "select transactions.id,lab.lab_name,'radiology' head,'lab_billing' module_no,lab_billing.patient_id,patients.patient_name,lab_billing.note,lab_billing.doctor_name,transactions.lab_billing_id as module_id,lab_billing.net_amount,transactions.payment_date,transactions.amount,(SELECT IFNULL(SUM(transactions.amount),0) from transactions WHERE transactions.lab_billing_id=lab_billing.id ) as paid_amount,staff.name,staff.surname,staff.employee_id,lab_test.test_name,lab_test.short_name" . $field_variable . " from lab_billing
             inner JOIN lab_report on lab_billing.id = lab_report.radiology_bill_id
               inner JOIN lab_test on lab_test.id = lab_report.radiology_id
               inner join lab on lab.id = lab_test.radiology_category_id
              LEFT JOIN transactions on lab_billing.id = transactions.lab_billing_id
              LEFT JOIN patients on patients.id = lab_billing.patient_id
              LEFT JOIN staff on staff.id = lab_report.collection_specialist " . $custom_join . "
              where 0=0 " . $condition . " and  date_format(lab_billing.date,'%Y-%m-%d')  >='" . $start_date . "'and date_format(lab_billing.date,'%Y-%m-%d') <= '" . $end_date . "' ";
        $this->datatables->query($sql)
            ->searchable('transactions.id,patients.patient_name' . $custom_field_column)
            ->orderable('transactions.id,transactions.payment_date,patients.patient_name,lab.lab_name,lab_test.test_name,lab_billing.doctor_name,staff.name,transactions.lab_billing_id,head,transactions.received_by,paid_amount' . $custom_field_column)
            ->sort('transactions.payment_date', 'desc')
            ->query_where_enable(true);
        return $this->datatables->generate('json');
    }
...
//new function
    ...
    {
        $query = $this->db->select('transactions.*,lab_billing.id as lab_billing_id,patients.note as pnote,patients.id as patient_id,patients.patient_name,patients.guardian_name,patients.gender,patients.id as patient_unique_id,patients.mobileno,patients.email,patients.dob,patients.image,patients.address')
            ->join("lab_billing", "lab_billing.id = transactions.lab_billing_id")
            ->join("patients", "patients.id = lab_billing.patient_id")
            ->where("lab_billing_id", $bill_id)
            ->where("transactions.patient_id", $patient_id)
            ->order_by("transactions.id", "desc")
            ->get("transactions");
        return $query->result_array();
    ...
    public function get_dailyTransaction($where)
    {
        $result = $this->db->select('sum(amount) as total')->from('transactions');
        $this->db->where($where);
        $result = $this->db->get()->row_array();
    ...
    public function ipd_bill_paymentbycase_id($case_id)
    {
        ...
        $ipd_bill_payment['radiology']['bill'] = $this->db->select('sum(net_amount) as total_bill')->from('lab_billing')->where('lab_billing.case_reference_id', $case_id)->get()->row_array();
        $ipd_bill_payment['radiology']['payment'] = $this->db->select('sum(amount) as total_payment')->from('transactions')->where(array('case_reference_id' => $case_id, 'lab_billing_id !=' => 'NULL'))->get()->row_array();
    ...
    public function opd_bill_paymentbycase_id($case_id)
    {
        ...
        $opd_bill_payment['radiology']['bill'] = $this->db->select('sum(net_amount) as total_bill')->from('lab_billing')->where('lab_billing.case_reference_id', $case_id)->get()->row_array();
        $opd_bill_payment['radiology']['payment'] = $this->db->select('sum(amount) as total_payment')->from('transactions')->where(array('case_reference_id' => $case_id, 'lab_billing_id !=' => 'NULL'))->get()->row_array();
