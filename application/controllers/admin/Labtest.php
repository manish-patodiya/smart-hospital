...
class Labtest extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('prefix_model', 'transaction_model1'));
    ...
    public function labviewDetail()
    {
        $id = $this->input->post("radiology_id");
        $result = $this->lab_test_model->getDetails($id);
        $data['result'] = $result;
    ...
    {
        $data = array();
        $categoryName = $this->lab_model1->getlabName();
        $data["categoryName"] = $categoryName;
        $data['charge_category'] = $this->charge_category_model->getCategoryByModule("lab");
        $parametername = $this->lab_model1->getlabparameter();
        $data["parametername"] = $parametername;
        // prd($data);
        $page = $this->load->view("admin/radio/_createRadiologyTest1", $data, true);
        echo json_encode(array('status' => 1, 'page' => $page));
    }
    ...
    public function editLabTest()
    {
        ...
        $data["categoryName"] = $this->lab_model1->getlabName();
        $parametername = $this->lab_model1->getlabparameter();
        //prd($parametername);
        $data["parametername"] = $parametername;

        if (!empty($result->lab_parameter)) {
            $total_rows = count($result->lab_parameter);
        }
        $page = $this->load->view("admin/radio/_editLabTest", $data, true);
        echo json_encode(array('status' => 1, 'page' => $page, 'total_rows' => $total_rows));
    }

    public function add()
    {
        if (!$this->rbac->hasPrivilege('lab_test', 'can_add')) {
            access_denied();
        }
        ...
        $this->form_validation->set_rules('test_name', $this->lang->line('test_name'), 'callback_lab_test|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            // prd($radiology);
            $insert_id = $this->lab_test_model->add($radiology, $insert_parameter_array, $update_parameter_array, $deleted_parameter_array);
    ...
    public function lab_test()
    {
        ...
        if ($test_name == "" && $short_name != "") {
            $this->form_validation->set_message('lab_test', $this->lang->line('the_test_name_field_is_required'));
            return false;
        }
        if ($short_name == "" && $test_name != "") {
            $this->form_validation->set_message('lab_test', $this->lang->line('the_short_name_field_is_required'));
            return false;
        }
        if ($short_name == "" && $test_name == "") {
            $this->form_validation->set_message('lab_test', $this->lang->line('the_test_name_and_short_name_required'));
            return false;
        }
        if ($test_name != '' && $short_name != '') {
            $count = $this->lab_test_model->test_uniqe($test_name, $short_name, $id);
            if ($count > 0) {
                $this->form_validation->set_message('lab_test', $this->lang->line('test_name_and_short_name_already_exist'));
                return false;
            }
        } else {
            return true;
    ...
    {
        $parametername = $this->lab_test_model->getpathoparameter();
        $data["parametername"] = $parametername;
        $detail = $this->lab_test_model->getparameterDetails($id);
        $data['detail'] = $detail;
    ...
    public function parameterdetails($id, $valueid = '')
    {
        $parametername = $this->lab_test_model->getpathoparameter();
        $data["parametername"] = $parametername;
        $detail = $this->lab_test_model->getparameterDetailsforpatient($valueid);
        $data['detail'] = $detail;
    ...
    public function addBill()
    {
        ...
        $radiology_billing_id = $this->input->post('lab_billing_id');
        ...
        $this->form_validation->set_rules('total', $this->lang->line('total'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('patient_name', $this->lang->line('patient'), 'trim|required|xss_clean');
        ...
        if ($radiology_billing_id == '') {
            $this->form_validation->set_rules(
                'amount', $this->lang->line('amount'), array('trim', 'required', 'xss_clean', 'valid_amount',
                    array('check_exists', array($this->lab_test_model, 'validate_paymentamount')),
                )
            );
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($patient_id)
                ...
                'generated_by' => $this->customlib->getLoggedInUserID(),
                    ...
                    $patient_id = $this->patient_model->add_patient($p_data);
                    // prd($patient_id);
                    $data = array(
                        'date' => ($bill_date),
                        'patient_id' => $patient_id,
            ...
            $inserted = $this->lab_test_model->addBill($data, $insert_array, $update_array, $array_delete, $radiology_billing_id, $transaction_data);
    ...
    public function prescriptionBill()
    {
        ...
        $lab_tests = $this->lab_test_model->getradiotestDetails();
    ...
    {
        if (!$this->rbac->hasPrivilege('lab_test', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'lab');
        $data = array();
        $data['fields'] = $this->customfield_model->get_custom_fields('labtest', 1);
        $categoryName = $this->lab_model1->getlabName();
        $data["categoryName"] = $categoryName;
        $data['charge_category'] = $this->lab_test_model->getChargeCategory();
        $parametername = $this->lab_model1->getlabparameter();
        $data["parametername"] = $parametername;
        $this->load->view('layout/header');
        $this->load->view('admin/lab_test/lab_search', $data);
    ...
    public function getparameterdetails()
    {
        ...
        $result = $this->lab_model1->getlabparameter($id);
    ...
    {
        if (!$this->rbac->hasPrivilege('lab_test', 'can_edit')) {
            access_denied();
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($data)) {
                $this->lab_test_model->updateparameter($condition);
            }
            if (!empty($insert_data)) {
                $this->lab_test_model->addparameter($insert_data);
            }
            if (!empty($delete_arr)) {
                $this->lab_test_model->delete_parameter($delete_arr);
            }
            $this->lab_test_model->update($radiology);
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        }
        echo json_encode($array);
    }
    ...
    {
        $id = $this->input->post('id');
        if (!$this->rbac->hasPrivilege('lab_test', 'can_delete')) {
            access_denied();
        }
        if (!empty($id)) {
            $this->lab_test_model->delete($id);
    ...
    public function deleteRadiologyBill()
    {
        ...
        if (!empty($id)) {
            $this->lab_test_model->deleteRadiologyBill($id);
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('delete_message'));
        } else {
            $array = array('status' => 'fail', 'error' => '', 'message' => '');
        }
    ...
    public function getRadiology()
    {
        ...
        $id = $this->input->post('radiology_id');
        $result = $this->lab_test_model->getRadiology($id);
    ...
    public function testReportBatch()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $bill_no = $this->lab_test_model->getMaxId();
            if (empty($bill_no)) {
                $bill_no = 0;
            }
            $bill = $bill_no + 1;
            ...
            $insert_id = $this->lab_test_model->testReportBatch($report_batch);
            $paramet_details = $this->lab_test_model->getparameterBypathology($id);
            foreach ($paramet_details as $pkey => $pvalue) {
                ...
                $insert_into_parameter = $this->lab_test_model->addParameterforPatient($paramet_insert_array);
            }
            ...
            if (isset($_FILES["radiology_report"]) && !empty($_FILES['radiology_report']['name'])) {
                ...
                $this->lab_test_model->testReportBatch($data_img);
            }
    ...
    public function getReportDetails($id, $parameter_id = '')
    {
        ...
        $result = $this->lab_test_model->getBillDetails($id);
        $data['result'] = $result;
        $detail = $this->lab_test_model->getAllBillDetails($id);
        $data['detail'] = $detail;
        $parameterdetails = $this->lab_test_model->getparameterDetailsforpatient($id);
    ...
    public function getBillDetails()
    {
        ...
        $result = $this->lab_test_model->getRadiologyBillByID($id);
    ...
    public function getradiobilldetails()
    {
        ...
        $print_details = $this->printing_model->get('', 'radiology');
        $data['print_details'] = $print_details;
        $result = $this->lab_test_model->getradioBillDetails($id);
        $data['result'] = $result;
        $detail = $this->lab_test_model->getAllradioBillDetails($id);
        $data['detail'] = $detail;
        $action_details = "";
    ...
    public function gettestreportbatch()
    {
        ...
        if (!$this->rbac->hasPrivilege('lab_bill', 'can_view')) {
            access_denied();
        }
        ...
        $testlist = $this->lab_test_model->getradiotestDetails();
        ...
        // prd($data);
        $this->load->view('layout/header');
        $this->load->view('admin/lab_test/labs', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function updatecollection()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->lab_test_model->updateTestReport($payment_array);

            $radiology_detail = $this->lab_test_model->getRadiologyReportByID($this->input->post('radiology_report_id'));
    ...
    public function updatereportparam()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($rows)) {
                ...
                $this->lab_test_model->addParameterforPatient($radiology_report_id, $approved_by, $approve_date, $insert_array, $update_array, $delete_array);
            }
            //==========
            if (isset($_FILES["attachment_report"]) && !empty($_FILES["attachment_report"]['name'])) {
                ...
                $this->lab_test_model->updateTestReport($data_img);
            }
            //==========
            ...
            $this->system_notification->send_system_notification('lab_test_report', $event_data);
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        }
        echo json_encode($array);
    }
    ...
    public function getRadiologyReportDetail()
    {
        ...
        $data["staff"] = $staff;
        $result = $this->lab_test_model->getPatientRadiologyReportDetails($id);
        $data['result'] = $result;
    ...
    public function assigntestradio()
    {
        ...
        $result = $this->lab_test_model->getBillNo();
        $id = $result["id"];
        if (!empty($result["id"])) {
            $bill_no = $id + 1;
        } else {
    ...
    {
        $dt_response = $this->lab_test_model->getAllradiologybillRecord();
    ...
    public function getPatientRadiologyDetails()
    {
        ...
        if (isset($is_bill)) {
            ...
            if ($this->rbac->hasPrivilege('lab_bill', 'can_edit')) {
            ...
            if ($this->rbac->hasPrivilege('lab_bill', 'can_delete')) {
    ...
    public function getReportCollectionDetail()
    {
        ...
        $report = $this->lab_test_model->getRadiologyReportByID($id);
    ...
    public function getlabtestDatatable()
    {
        $dt_response = $this->lab_test_model->getAlllabtest();
        $fields = $this->customfield_model->get_custom_fields('labtest', 1);
    ...
    public function getRadiologyReport()
    {
        ...
        $id = $this->input->post('id');
        $result = $this->lab_test_model->getRadiologyReport($id);
    ...
    public function updateTestReport()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->lab_test_model->updateTestReport($report_batch);
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($parameter_id)) {
                foreach ($parameter_id as $pkey => $pvalue) {
                    ...
                    $this->lab_test_model->addparametervalue($parameter_value_arr);
                    $i++;
                }
            }
    ...
    public function deleteTestReport($id)
    {
        ...
        $this->lab_test_model->deleteTestReport($id);
    }
    ...
    public function deletetestbill($id)
    {
        ...
        $this->lab_test_model->deletetestbill($id);
    }
    ...
    public function radiologyReport()
    {
        ...
        $data['testlist'] = $this->lab_test_model->getradiotestDetails();
        $radiologist = $this->staff_model->getStaffbyrole(6);
        $data["radiologist"] = $radiologist;
        $categoryName = $this->lab_model1->getlabName();
        $data["categoryName"] = $categoryName;
    ...
    public function radiologyreports()
    {
        ...
        $reportdata = $this->transaction_model1->radiologybillreportsRecord($condition);
    ...
    public function getRadiologyTransaction()
    {
        ...
        $is_bill = $this->input->post('is_bill');
        $radiology_transaction = $this->transaction_model1->radiologyPayments($radiology_billing_id);
        if (isset($is_bill)) {
            $data['is_bill'] = true;
        ...
        $radio_billing = $this->lab_test_model->getRadiologyBillByID($radiology_billing_id);
        $data['radio_billing'] = $radio_billing;

        $data["lab_billing_id"] = $radiology_billing_id;
        $data["payment_mode"] = $this->payment_mode;
        $data['radiology_transaction'] = $radiology_transaction;
        $page = $this->load->view("admin/lab_test/_getLabTransaction", $data, true);
        echo json_encode(array('status' => 1, 'page' => $page));
    }
...
//======
            $radiology_billing_id = $this->input->post('lab_billing_id');
            $radiology_billing_detail = $this->transaction_model1->radiologyTotalPayments($radiology_billing_id);
            ...
            if ($net_amount >= ($total_paid + $amount_paying)) {
                ...
                $payment_array = array(
                    ...
                    'patient_id' => $this->input->post('patient_id'),
                    'section' => $payment_section['radiology'],
                    'lab_billing_id' => $this->input->post('lab_billing_id'),
                    'payment_mode' => $this->input->post('payment_mode'),
                    'note' => $this->input->post('note'),
                ...
                $this->transaction_model1->add($payment_array);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
            } else {
    ...
    public function editradiology()
    {
        ...
        $id = $this->input->post('id');
        $radiology_data = $this->lab_test_model->getRadiologyBillByID($id);
        ...
        $data["doctors"] = $doctors;
        $data["payment_mode"] = $this->payment_mode;
        $page = $this->load->view("admin/radio/_editradiology1", $data, true);
        $total_rows = count($radiology_data->lab_report);
    ...
    public function printTransaction()
    {
        ...
        $id = $this->input->post('id');
        $charge = array();
        $transaction = $this->transaction_model1->radiologyPaymentByTransactionId($id);
        $data['print_details'] = $print_details;
    ...
    public function printtestparameterdetail()
    {
        ...
        $id = $this->input->post('id');
        $data['id'] = $id;
        $data['head_result'] = $this->lab_test_model->getRadiologyBillByID($id);
        $result = $this->lab_test_model->printtestparameterdetail($id);
        $data['bill_prefix'] = $this->customlib->getSessionPrefixByType('radiology_billing');
        $data['result'] = $result;
    ...
    {
        $this->load->view('layout/header');
        $this->load->view('admin/radio/labs');
        $this->load->view('layout/footer');
    }
}