...
class Patient_model extends MY_Model
{
    ...
    {
        $this->db->select('lab_report.generated_by,staff.id as staffid,staff.name as staffname, staff.surname as staffsurname,staff.employee_id');
        $this->db->join('staff', 'staff.id=lab_report.generated_by', 'left');
        $this->db->group_by('lab_report.generated_by');
    ...
    public function searchAll($searchterm)
    {
        ...
        foreach ($result as $key => $value) {
            if ($value['is_active'] == 'yes') {
                ...
                $info_url[2] = base_url() . 'admin/labtest/getTestReportBatch';
    ...
    public function searchAlldisable($searchterm)
    {
        ...
        foreach ($result as $key => $value) {
            if ($value['is_active'] == 'no') {
                ...
                $info[0] = $this->db->where("patient_id", $id)->get("opd_details");
                $info[1] = $this->db->where("patient_id", $id)->get("ipd_details");
                $info[2] = $this->db->where("patient_id", $id)->get("lab_report");
    ...
    public function getRadiologyEarning($search = '')
    {
        ...
        $query = $this->db->select('sum(lab_report.apply_charge) as amount')
            ->join('lab_test', 'lab_test.charge_id = charges.id')
            ->join('lab_report', 'lab_report.radiology_id = lab_test.id')
            ->get('charges');
        $result = $query->row_array();
        return $result["amount"];
    }
    ...
    public function getPatientRadiologyVisitDetails($patient_id)
    {
        ...
        $this->datatables
            ->select('lab_billing.*,(SELECT IFNULL(SUM(transactions.amount),0) from transactions WHERE transactions.radiology_billing_id=lab_billing.id ) as paid_amount,patients.patient_name,patients.id as pid,staff.name,staff.surname,staff.employee_id')
            ->join('patients', 'patients.id = lab_billing.patient_id', 'left')
            ->join('transactions', 'transactions.radiology_billing_id = lab_billing.id', 'left')
            ->join('staff', 'staff.id = lab_billing.doctor_id', 'left')
            ->where('patients.id', $patient_id);
        $result = $this->db->get('lab_billing');
        return $result->result_array();
    }
    ...
    {
        $result = $this->db->select("'Radiology' as module,patients.patient_name,patients.id as patient_id,lab_billing.id as bill_no,lab_billing.net_amount as charge")
            ->join("patients", "patients.id = lab_billing.patient_id", "left")
            ->where("lab_billing.case_reference_id", $case_reference_id)
            ->get("lab_billing")
            ->result();
        foreach ($result as $key => $value) {
    ...
    public function getallinvestigation($case_reference_id)
    {
        ...
        $query = $this->db->query("select pathology_report.id as report_id,pathology_report.pathology_bill_id,pathology.test_name,pathology.short_name,pathology.report_days,pathology.id as pid,pathology.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, 'pathology' as type, pathology_report.pathology_center as test_center, pathology_report.collection_date,pathology_report.reporting_date,pathology_report.parameter_update from pathology_billing inner join pathology_report on pathology_report.pathology_bill_id = pathology_billing.id inner join pathology on pathology_report.pathology_id = pathology.id left join staff on staff.id = pathology_billing.doctor_id left join staff as collection_specialist_staff on collection_specialist_staff.id = pathology_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = pathology_report.approved_by where pathology_billing.case_reference_id= " . $case_reference_id . "
            union all
            select lab_report.id as report_id, lab_report.radiology_bill_id,lab_test.test_name,lab_test.short_name,lab_test.report_days,lab_test.id as pid,lab_test.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, 'radiology' as type,lab_report.radiology_center as test_center,lab_report.collection_date,lab_report.reporting_date,lab_report.parameter_update  from lab_billing inner join lab_report on lab_report.radiology_bill_id = lab_billing.id inner join lab_test on lab_report.radiology_id = lab_test.id left join staff on staff.id = lab_report.consultant_doctor left join staff as collection_specialist_staff on collection_specialist_staff.id = lab_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = lab_report.approved_by where lab_billing.case_reference_id=" . $case_reference_id . " ");
        $result = $query->result_array();
        return $result;
    }
    ...
    public function allinvestigationbypatientid($patient_id)
    {
        ...
        $query = $this->db->query("select pathology_report.id as report_id,pathology_report.pathology_bill_id,pathology.test_name,pathology.short_name,pathology.report_days,pathology.id as pid,pathology.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, 'pathology' as type, pathology_report.pathology_center as test_center, pathology_report.collection_date,pathology_report.reporting_date,pathology_report.parameter_update,pathology_billing.case_reference_id from pathology_billing inner join pathology_report on pathology_report.pathology_bill_id = pathology_billing.id inner join pathology on pathology_report.pathology_id = pathology.id left join staff on staff.id = pathology_billing.doctor_id left join staff as collection_specialist_staff on collection_specialist_staff.id = pathology_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = pathology_report.approved_by where pathology_billing.patient_id= " . $patient_id . "
            union all
            select lab_report.id as report_id, lab_report.radiology_bill_id,lab_test.test_name,lab_test.short_name,lab_test.report_days,lab_test.id as pid,lab_test.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, 'radiology' as type,lab_report.radiology_center as test_center,lab_report.collection_date,lab_report.reporting_date,lab_report.parameter_update,lab_billing.case_reference_id  from lab_billing inner join lab_report on lab_report.radiology_bill_id = lab_billing.id inner join lab_test on lab_report.radiology_id = lab_test.id left join staff on staff.id = lab_report.consultant_doctor left join staff as collection_specialist_staff on collection_specialist_staff.id = lab_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = lab_report.approved_by where lab_billing.patient_id=" . $patient_id);
        $result = $query->result_array();
        return $result;
    }
    ...
    public function getlabinvestigation($patient_id)
    {
        ...
        $query = $this->db->query("select pathology.test_name,pathology.short_name
            ...
            inner join pathology  on pathology_report.pathology_id = pathology.id
             ...
             union all
             select lab_test.test_name,lab_test.short_name
            from lab_billing
            inner join lab_report  on lab_billing.id = lab_report.radiology_bill_id
            inner join lab_test  on lab_report.radiology_id =lab_test.id
             where lab_billing.patient_id='" . $patient_id . "'  ");
    ...
    public function getlabinvestigationbycaseid($case_reference_id)
    {
        ...
        $query = $this->db->query("select pathology.test_name,pathology.short_name
            ...
            from
            opd_details
             left join  lab_billing on opd_details.patient_id = lab_billing.patient_id
            inner join lab_report  on lab_billing.id = lab_report.radiology_bill_id
            inner join lab_test  on lab_report.radiology_id =lab_test.id
             where opd_details.case_reference_id ='" . $case_reference_id . "'  ");
    ...
    public function getpatientoverview($patient_id)
    {
        ...
        $query = $this->db->query("select pathology.test_name,pathology.short_name  from pathology_billing  inner join pathology_report  on pathology_billing.id= pathology_report.pathology_bill_id  inner join pathology  on pathology_report.pathology_id = pathology.id
             where pathology_billing.patient_id='" . $patient_id . "'  union all  select lab_test.test_name,lab_test.short_name from lab_billing  inner join lab_report  on lab_billing.id = lab_report.radiology_bill_id  inner join lab_test  on lab_report.radiology_id =lab_test.id  where lab_billing.patient_id='" . $patient_id . "' limit 5  ");
    ...
    public function getpatientoverviewbycaseid($case_reference_id)
    {
        ...
        $query = $this->db->query("select pathology.test_name,pathology.short_name   from opd_details  left join  pathology_billing on opd_details.patient_id = pathology_billing.patient_id   inner join pathology_report  on pathology_billing.id= pathology_report.pathology_bill_id   inner join pathology  on pathology_report.pathology_id = pathology.id
             where opd_details.case_reference_id ='" . $case_reference_id . "'
             union all  select lab_test.test_name,lab_test.short_name   from opd_details   left join  lab_billing on opd_details.patient_id = lab_billing.patient_id  inner join lab_report  on lab_billing.id = lab_report.radiology_bill_id  inner join lab_test  on lab_report.radiology_id =lab_test.id   where opd_details.case_reference_id ='" . $case_reference_id . "'  ");
    ...
    public function getcaseid($data)
    {
        $this->db->select('ipd_details.patient_id,ipd_details.case_reference_id,p.id,p.patient_name');
        $this->db->join('patients p', 'p.id = ipd_details.patient_id', "inner");
        if ($data == "female") {
            $this->db->where('p.gender', $data);
        }
        $this->db->where('ipd_details.discharged', "no");
