            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title titlefix"><?php echo $this->lang->line('lab_test'); ?></h3>
                        <div class="box-tools pull-right">
                            <?php if ($this->rbac->hasPrivilege('lab_test', 'can_add')) {?>
                            <button class="btn btn-primary btn-sm radiology addtest"
                                data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> <?php echo $this->lang->line('please_wait'); ?>"><i
                                    class="fa fa-plus"></i>
                                <?php echo $this->lang->line('add_lab_test'); ?></button>
                            <?php }?>

                        </div>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <div class="download_label"><?php echo $this->lang->line('lab_test'); ?></div>
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover ajaxlist" cellspacing="0"
                                width="100%" data-export-title="<?php echo $this->lang->line('lab_test'); ?>">
                                <thead>
                ...
                <div class="modal-footer">
                    <div class="pull-right">
                        <button type="submit" data-loading-text="<?php echo $this->lang->line('processing'); ?>"
                            id="formaddbtn" class="btn btn-info"><i class="fa fa-check-circle"></i>
...
$(document).on('click', '.addtest', function() {
    ...
    $this.button('loading');
    $.ajax({
        url: '<?php echo base_url(); ?>admin/labtest/createRadiologyTest',
        type: "POST",
        dataType: 'json',
        beforeSend: function() {
            $this.button('loading');
...
$(document).ready(function(e) {
    $("#formadd").on('submit', (function(e) {
        ...
        e.preventDefault();
        $.ajax({
            url: '<?php echo base_url(); ?>admin/labtest/add',
            type: "POST",
...
function getRecord(id) {
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#updateid").val(id);
            //console.log(data);
            $('select[id="radiology_category_id"] option[value="' + data.radiology_category_id + '"]').attr(
                "selected", "selected");
...
function delete_record(id) {
    if (confirm('<?php echo $this->lang->line('delete_confirm'); ?>')) {
        $.ajax({
            url: '<?php echo base_url(); ?>admin/labtest/delete/',
            type: "POST",
            data: {
                id: id
            },
...
function viewDetail(id) {
    $.ajax({
        url: '<?php echo base_url(); ?>admin/labtest/labviewDetail',
        type: "POST",
        data: {
            radiology_id: id
        },
        ...
        success: function(data) {
            ...
            $('#edit_delete').html(
                "<?php if ($this->rbac->hasPrivilege('lab_test', 'can_edit')) {?><a href='#' class='edittest' data-record-id='" +
                id +
                "'  data-loading-text='<i class= \"fa fa-circle-o-notch fa-spin\"></i>' data-toggle='tooltip'   data-original-title='<?php echo $this->lang->line('edit'); ?>'><i class='fa fa-pencil'></i></a><?php }if ($this->rbac->hasPrivilege('lab_test', 'can_delete')) {?><a onclick='delete_record(" +
                id +
...
$(document).on('click', '.add-record', function() {
    ...
    var div = "<td width='35%'><input type='hidden' name='total_rows[]' value='" + radiology_parmeter_rows +
        "'><input type='hidden' name='inserted_id_' value='0'><select class='form-control select2 radiology_parmeter' name='parameter_name_" +
        radiology_parmeter_rows +
        ...
        radiology_parmeter_rows + "' readonly id='reference_range" + radiology_parmeter_rows +
        "' class='form-control reference_range'></td><td width='30%'><input type='text' name='radio_unit_" +
        radiology_parmeter_rows + "' readonly id='radio_unit" + radiology_parmeter_rows +
        "' class='form-control radio_unit'></td>";

    var row = table.insertRow(table_len).outerHTML = "<tr id='row" + radiology_parmeter_rows + "'>" + div +
...
function getparameterdetails(pathology_parmeter_obj, parameter_id) {
    ...
    $.ajax({
        type: "POST",
        url: base_url + "admin/labtest/getparameterdetails",
        data: {
            'id': parameter_id
        },
        dataType: 'json',
...
$(document).on('click', '.edittest', function() {
    ...
    $this.button('loading');
    $.ajax({
        url: '<?php echo base_url(); ?>admin/labtest/editLabTest',
        type: "POST",
        data: {
            'id': record_id
        },
...
(function($) {
    'use strict';
    $(document).ready(function() {
        initDatatable('ajaxlist', 'admin/labtest/getlabtestDatatable', [], [], 100,
            [{
                "sWidth": "100px",
                "bSortable": false,
                "aTargets": [-1, -2, -3],
