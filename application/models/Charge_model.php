...
class Charge_model extends MY_Model
{
    ...
    {
        $query = $this->db->select('charges.*')->join('charge_categories', 'charge_categories.id = charges.charge_category_id', 'inner')->where('lower(charge_categories.name)', 'Admission')->get('charges');
        return $query->result();
    }

    public function getBedCharge()
    {
        $query = $this->db->select('charges.*')->join('charge_categories', 'charge_categories.id = charges.charge_category_id', 'inner')->where('lower(charge_categories.name)', 'bed charges')->get('charges');
        return $query->result();
    }
