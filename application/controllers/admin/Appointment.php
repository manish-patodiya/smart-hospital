...
class Appointment extends Admin_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $consult = 0; //$this->input->post('live_consult') ?: 0;
            $cheque_date = $this->customlib->dateFormatToYYYYMMDD($this->input->post("cheque_date"));
            $doc_id = $this->input->post('doctorid');
            $token = $this->appointment_model->generateToken($this->customlib->dateFormatToYYYYmmdd($date, $this->time_format), $doc_id);
            ...
            $visit_details = array(
                'appointment_date' => $date_appoint, //date("Y-m-d H:i:s"),
                'opd_details_id' => 0,
    ...
    {
        $data1 = $this->setting_model->getSetting();
        $data['res'] = $data1;
        ...
        if($crf_id != "")
        {        
            $result = $this->appointment_model->getDetailsAppointment('', '', $crf_id);
            // prd($result);
        }      
        ...
        $result["doctor_shift_name"] = $result["doctor_shift_name"];
    ...
    public function getappointmentdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                //==============================
                $row[] = "<i class='text-green " . ($value->done ? 'fas fa-check-circle' : 'far fa-circle') . "  fa-lg' style='cursor:pointer' app_id='$value->id' is_visited ='$value->done' onclick='checkVisits(this)'></i>";
