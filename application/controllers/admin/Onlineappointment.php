...
class Onlineappointment extends Admin_Controller
{
    ...
    public function patientSchedule()
    {
        ...
        $data["yesno_condition"] = $this->config->item('yesno_condition');
    ...
    public function doctors()
    {
        ...
        $data['payment_mode'] = $this->payment_mode;
        $data['appoint_priority_list'] = $this->appoint_priority_model->appoint_priority_list();
        $this->load->view('admin/onlineappointment/doctors', $data, );
    ...
    public function setVisit()
    {
        ...
        $done = $this->onlineappointment_model->setVisit($f_val, $id);
        echo json_encode([
            "status" => 1,
            'data' => $done,
        ]);
    ...
    {
        $data = $this->staff_model->getDoctors(3);
        // prd($data);
        $doctors = [];
        foreach ($data as $k => $v) {
            $doctors[$v->name][] = $v;
    ...
    {
        $date = date('y-m-d');
        $data = $this->onlineappointment_model->getAll($date);

        $doctors = [];
        $drs = $this->staff_model->getDoctors(3);
        foreach ($drs as $dr) {
            $id = $dr->id;
