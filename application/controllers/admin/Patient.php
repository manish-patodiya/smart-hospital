...
class patient extends Admin_Controller
{
    ...
    public function addpatient()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $patient_data = array(
                'patient_name' => ucfirst($this->input->post('name')),
                'mobileno' => $this->input->post('mobileno'),
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('opd_print_bill', 'can_view')) {
                    $action .= "<a href='javascript:void(0)' data-loading-text='<i class=\"fa fa-circle-o-notch fa-spin\"></i>' data-opd-id=" . $case_reference_id . " data-record-id=" . $visit_details_id . " class='btn btn-default btn-xs print_visit_bill'  data-toggle='tooltip' title='" . $this->lang->line('print_bill') . "'><i class='fa fa-file'></i></a>";
                }
    ...
    public function ipdsearch($bedid = '', $bedgroupid = '')
    {
        ...
        $data['admission_list'] = $this->charge_model->getAdmissionCharge();
    ...
    public function ipdprofile($ipdid)
    {
        ...
        $data['bed_charges'] = $this->charge_model->getBedCharge();
    ...
    public function add_inpatient()
    {
        ...
        $this->form_validation->set_rules('patient_name', $this->lang->line('patient_name'), 'trim|required|callback_valid_patient');
        $this->form_validation->set_rules('age[year]', $this->lang->line('year'), 'trim|required|xss_clean|numeric');
        $this->form_validation->set_rules('age[month]', $this->lang->line('month'), 'trim|required|xss_clean|numeric');
        $this->form_validation->set_rules('age[day]', $this->lang->line('day'), 'trim|required|xss_clean|numeric');
        $this->form_validation->set_rules('gender', $this->lang->line('gender'), 'trim|required');
        $this->form_validation->set_rules('mobileno', $this->lang->line('phone'), 'trim|required');
        $this->form_validation->set_rules('guardian', $this->lang->line('guardian'), 'trim|required');
        ...
        $this->form_validation->set_rules('charge_id', 'Admission Charge', 'trim|required');
        $this->form_validation->set_rules('admission_fees', $this->lang->line('amount'), 'trim|required');
        ...
        if ($this->form_validation->run() == false) {
            $msg = array(
                ...
                'consultant_doctor' => form_error('consultant_doctor'),
                'charge_id' => form_error('charge_id'),
                'admission_fees' => form_error('admission_fees'),
            );
            ...
            if ($patient_id) {
                ...
                $this->patient_model->add_patient($p_data);
            } else {
                $p_data = [
                    'patient_name' => $patient_name,
                    'age' => $age['year'],
            ...
            $amount = $this->input->post('admission_fees');

            // INSERT ADMISSION CHARGE
            ...
            $insert_data = array(
                ...
                'ipd_id' => $ipd_id,
                'apply_charge' => $amount,
                'standard_charge' => $amount,
                'amount' => $amount,
                'created_at' => date('Y-m-d'),
            );
            $this->charge_model->add_charges($insert_data);

            // INSERT FIRST ADMISSION PAYMENT
            // $payment_section = $this->config->item('payment_section');
    ...
    public function getBedStatusIPD()
    {
        $floor_list = $this->floor_model->floor_list();
        $bedlist = $this->bed_model->bed_list();
        $bedactive = $this->bed_model->bed_active();
        ...
        $data["bedgroup_list"] = $bedgroup_list;
        $data['bedactive'] = $bedactive;
        $this->load->view("admin/patient/bedStatusForIPD", $data);
    }
    ...
    public function updateBed()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $case_reference_id = $this->input->post('case_ref_id');
            $change_date = $this->customlib->dateFormatToYYYYMMDDHis($this->input->post('change_date'), $this->time_format);
            // $getLastBed = $this->bed_model->getLastBed($case_reference_id);
            // $fdate = $getLastBed->from_date;
            // $days = (new DateTime($fdate))->diff(new DateTime($change_date))->d;

            $data = array(
                'id' => $this->input->post('ipd_id'),
            ...
            if ($case_reference_id) {
                $insert_data = array(
                    ...
                    'ipd_id' => $this->input->post('ipd_id'),
                    'apply_charge' => $this->input->post('amount'),
                    'standard_charge' => $this->input->post('amount'),
                    'amount' => $this->input->post('amount'),
                    'created_at' => date('Y-m-d'),
                );
                $this->charge_model->add_charges($insert_data);

                $bed_history_data['case_reference_id'] = $case_reference_id;
                $bed_history_data['to_date'] = $change_date;
                $bed_history_data['is_active'] = "no";

                $this->bed_model->updateBedHistory($bed_history_data);

                $bed_data = array('id' => $this->input->post("oldbedno"), 'is_active' => 'yes');
                $this->bed_model->savebed($bed_data);

                $bed_history = array(
                    "case_reference_id" => $case_reference_id,
                    "bed_group_id" => $this->input->post("bedgroup"),
                    "bed_id" => $this->input->post("bedno"),
                    "from_date" => date("Y-m-d H:i:s"),
                    "is_active" => "yes",
                );
                $this->bed_model->saveBedHistory($bed_history);

                $bed_data = array('id' => $this->input->post("bedno"), 'is_active' => 'no');
                $this->bed_model->savebed($bed_data);
            }
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        }
