...
class Appointment_model extends MY_Model
{
    ...
    //========================================================================================
    public function generateToken($date, $doc_id)
    {
        $query = $this->db->query("SELECT MAX(token) as token from appointment where CONVERT(date,date) = '$date' AND doctor = '$doc_id'");
        $res = $query->result();
        return ($res[0]->token) + 1;
    }
    ...
    public function getAllappointmentRecord()
    {
        ...
        $fdate = $this->input->get('date') ?: date('d-m-Y');
        $userdata = $this->customlib->getUserData();
        $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'];
        ...
        $this->datatables
            ...
            ->orderable('patients.patient_name,appointment.id,appointment.token,appointment.date,patients.mobileno,patients.gender,staff.name,appointment.source,appoint_priority.appoint_priority,appointment.live_consult' . $custom_field_column . ', appointment_payment.paid_amount')
            ->sort('appointment.date', 'desc')
            ->from('appointment');
        if ($doc_id) {
            $this->datatables->where('appointment.doctor', $doc_id);
        }
        if ($fdate) {
            $this->datatables->where('date(appointment.date)', date("Y-m-d", strtotime($fdate)));
        }
    ...
    public function getDetailsAppointment($id = null, $is_patient = null, $crf_id = null)
    {
        ...
        if(isset($crf_id))
        {        
            $this->db->where('appointment.case_reference_id', $crf_id);
        }        
        else{
            $this->db->where('appointment.id', $id);
        }
