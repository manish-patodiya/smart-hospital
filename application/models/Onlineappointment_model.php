...
class Onlineappointment_model extends MY_Model
{
    ...
    public function getPatient($doctor_id, $date)
    {
        $query = $this->db->select('patients.id,patients.patient_name,appointment.date,appointment.time,')
            ->join("patients", "appointment.patient_id=patients.id")
            ->where("appointment.doctor=$doctor_id")->from("appointment");
        $query = $this->db->get();
    ...
    {
        $query = $this->db->select('patients.id,patients.patient_name,appointment.id,appointment.token,appointment.date,appointment.doctor,appointment.patient_id,appointment.done,staff.id,staff.name,staff.surname,')
            ->join("patients", "patients.id=appointment.patient_id")
            ->join("staff", "staff.id=appointment.doctor")
    ...
    {
        $query = $this->db->select('appointment.doctor,appointment.id,staff.id,staff.name,staff.surname')->join("staff", "staff.id=appointment.doctor")
            ->get("appointment");
        $result = $query->result();
        return $result;
    }
    public function setVisit($data, $id)
    {
